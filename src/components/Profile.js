...
const [isPasswordUpdated, setIsPasswordUpdated] = useState(false);
  ...
  const handleSaveProfile = async () => {
    try {
      const response = await axios.post(
        'http://localhost:22222/api/user/update-profile',
        { fullName: newFullName }, // Ensure this is correctly set
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      console.log(response.data); // Log response for success or error message
  ...
  const handlePasswordChange = async () => {
    ...
    try {
      const response = await axios.post(
        'http://localhost:22222/api/user/update-password',
